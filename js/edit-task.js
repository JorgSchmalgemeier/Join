...
function checkForChosenPrioEditTask(idToOpen) {
    if (tasks[idToOpen]['prio'] == 'Urgent') {
        updateChosenPrioEditTask('prio-div1', 'bg-red', 'prio-img1', 'become-white', 'Urgent');
    }
    if (tasks[idToOpen]['prio'] == 'Medium') {
        updateChosenPrioEditTask('prio-div2', 'bg-orange', 'prio-img2', 'become-white', 'Medium');
        
    }
    if (tasks[idToOpen]['prio'] == 'Low') {
        updateChosenPrioEditTask('prio-div3', 'bg-green', 'prio-img3', 'become-white', 'Low');
    }
}


/**
 * This function updates the prio decision and make it visible for the user in frontend
 * 
 * @param {*} prioDiv - The div which should be change the color
 * @param {*} cssClass - CSS class with the color
 * @param {*} prioImg - The img for the chosen category
 * @param {*} cssClassImg - CSS class to make the img white
 * @param {*} prio - The chosen prio
 */
function updateChosenPrioEditTask(prioDiv, cssClass, prioImg, cssClassImg, prio) {
    document.getElementById(prioDiv).classList.add(cssClass);
    document.getElementById(prioImg).classList.add(cssClassImg);
    chosenPrio = prio;
}
