...
async function moveTo(category) {
    let currentIndex = tasks.findIndex(task => task['id'] == currentDraggedElement)
...
function createProgressBarForTask(element) {
    ...
    document.getElementById(`progress-bar${element['id']}`).style = `width: ${width}%`;
    document.getElementById(`progress-bar-text${element['id']}`).innerHTML = `${done}/${length} Done`;

    if (length == 0) {
        document.getElementById(`progress-bar-div${element['id']}`).classList.add('d-none');
